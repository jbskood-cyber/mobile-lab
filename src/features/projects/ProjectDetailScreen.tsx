...
export function ProjectDetailScreen() {
  ...
  if (!project || !metrics) return <SafeAreaView style={styles.safe}><View style={styles.missing}><Text style={styles.missingTitle}>Este proyecto ya no existe</Text><Pressable onPress={() => router.back()} style={styles.primary}><Text style={styles.primaryText}>Volver</Text></Pressable></View></SafeAreaView>;
    ...
    <>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.projectHeader}><View style={styles.projectIcon}><FocoIcon name={project.icon as IconName} size={27} color={foco.colors.text} /></View><View style={styles.projectCopy}><Text style={styles.title}>{project.name}</Text>{project.description ? <Text style={styles.description}>{project.description}</Text> : null}</View></View>
          <View style={styles.metrics}><Metric value={`${Math.round(metrics.progress * 100)}%`} label="Progreso" /><Metric value={`${metrics.completedPomodoros}/${metrics.plannedPomodoros}`} label="Pomodoros" /><Metric value={formatDuration(metrics.focusSeconds, true)} label="Enfoque" /></View>
          ...
          <View style={styles.list}>{openTasks.length > 0 ? openTasks.map((task) => <TaskRow key={task.id} task={task} projectName={projectMap.get(task.projectId) ?? project.name} completedPomodoros={pomodoros.get(task.id) ?? 0} onPress={() => router.push({ pathname: '/task/[id]', params: { id: task.id } })} onToggle={() => toggleTask(task)} />) : <Empty text="No quedan tareas pendientes." />}</View>

          <Pressable accessibilityRole="button" onPress={() => setShowCompleted((value) => !value)} style={({ pressed }) => [styles.completedHeader, pressed && pressedStyle]}><View><Text style={styles.sectionTitle}>Completadas</Text><Text style={styles.sectionDetail}>{completedTasks.length} tareas</Text></View><FocoIcon name={showCompleted ? 'chevron-down' : 'chevron-right'} size={18} color={foco.colors.muted} /></Pressable>
...
function Metric({ value, label }: { value: string; label: string }) { return <View style={styles.metric}><Text style={styles.metricValue} numberOfLines={1} adjustsFontSizeToFit>{value}</Text><Text style={styles.metricLabel}>{label}</Text></View>; }
function Action({ icon, label, onPress, primary = false }: { icon: 'plus' | 'play' | 'archive'; label: string; onPress: () => void; primary?: boolean }) { return <Pressable onPress={onPress} style={({ pressed }) => [styles.action, primary && styles.actionPrimary, pressed && pressedStyle]}><FocoIcon name={icon} size={20} color={primary ? foco.colors.bg : foco.colors.text} /><Text style={[styles.actionText, primary && styles.actionTextPrimary]}>{label}</Text></Pressable>; }
function SectionHeader({ title, detail }: { title: string; detail: string }) { return <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>{title}</Text><Text style={styles.sectionDetail}>{detail}</Text></View>; }
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: foco.colors.text, fontSize: 16, fontWeight: '600' },
  ...
  metric: { flex: 1 },
  metricValue: { color: foco.colors.text, fontSize: 19, fontWeight: '600', fontVariant: ['tabular-nums'] },
  ...
  sectionTitle: { color: foco.colors.text, fontSize: 17, fontWeight: '600' },
  ...
  sessionTitle: { color: foco.colors.text, fontSize: 14.5, fontWeight: '500' },
  ...
  missingTitle: { color: foco.colors.text, fontSize: 20, fontWeight: '600' },
