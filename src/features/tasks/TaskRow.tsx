    ...
    <View style={styles.row}>
      ...
      >
        <View style={[styles.checkCircle, task.completed && styles.checkCircleDone]}>
          {task.completed ? <FocoIcon name="check" size={15} color={foco.colors.bg} strokeWidth={2.4} /> : null}
        </View>
      </Pressable>
      <Pressable accessibilityRole="button" accessibilityLabel={`Abrir ${task.title}`} onPress={onPress} style={({ pressed }) => [styles.content, pressed && styles.pressed]}>
        <View style={styles.titleLine}>
          ...
          {task.favorite ? <FocoIcon name="star" size={16} color={foco.colors.text} /> : null}
        </View>
        <View style={styles.metaRow}>
          <Text style={styles.project} numberOfLines={1}>{projectName}</Text>
          <View style={styles.dot} />
          <Text style={[styles.schedule, overdue && styles.overdue]}>{getTaskScheduleLabel(task)}</Text>
          {task.estimatedPomodoros > 0 ? <><View style={styles.dot} /><Text style={styles.pomodoros}>{completedPomodoros}/{task.estimatedPomodoros} foco</Text></> : null}
          {task.subtasks.length > 0 ? <><View style={styles.dot} /><Text style={styles.pomodoros}>{subtaskDone}/{task.subtasks.length}</Text></> : null}
        </View>
      </Pressable>
      <Pressable accessibilityRole="button" accessibilityLabel={`Opciones de ${task.title}`} onPress={onPress} style={({ pressed }) => [styles.trailing, pressed && pressedStyle]}>
...
const styles = StyleSheet.create({
  ...
  title: { flex: 1, color: foco.colors.text, fontSize: 16, lineHeight: 21, fontWeight: '550' },
