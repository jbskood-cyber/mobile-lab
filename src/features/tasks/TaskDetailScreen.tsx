...
export function TaskDetailScreen() {
  ...
  if (!task) {
    return <SafeAreaView style={styles.safe}><View style={styles.missing}><Text style={styles.missingTitle}>Esta tarea ya no existe</Text><Pressable onPress={() => router.back()} style={styles.primary}><Text style={styles.primaryText}>Volver</Text></Pressable></View></SafeAreaView>;
  }
    ...
    <>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          {task.subtasks.length > 0 ? <Section title="Subtareas" detail={`${task.subtasks.filter((item) => item.completed).length}/${task.subtasks.length}`}>{task.subtasks.map((subtask) => <Pressable key={subtask.id} onPress={() => toggleSubtask(task.id, subtask.id)} style={({ pressed }) => [styles.subtask, pressed && styles.pressed]}><View style={[styles.smallCheck, subtask.completed && styles.bigCheckDone]}>{subtask.completed ? <FocoIcon name="check" size={12} color={foco.colors.bg} /> : null}</View><Text style={[styles.subtaskText, subtask.completed && styles.titleDone]}>{subtask.title}</Text></Pressable>)}</Section> : null}
          {task.notes ? <Section title="Notas"><Text style={styles.notes}>{task.notes}</Text></Section> : null}
          <Section title="Actividad" detail={`${sessions.length} sesiones`}>{sessions.length > 0 ? sessions.slice(0, 8).map((session) => <View key={session.id} style={styles.sessionRow}><View><Text style={styles.sessionDate}>{new Date(session.endedAt).toLocaleDateString('es-MX', { day: 'numeric', month: 'short' })}</Text><Text style={styles.sessionMode}>{session.mode === 'pomodoro' ? `Pomodoro · ciclo ${session.cycleNumber}` : 'Cronómetro'}</Text></View><Text style={styles.sessionDuration}>{formatDuration(session.durationSec, true)}</Text></View>) : <Text style={styles.emptyCopy}>Todavía no has enfocado tiempo en esta tarea.</Text>}</Section>
...
function recurrenceLabel(kind: string) { return ({ daily: 'Diaria', weekdays: 'Días laborables', weekly: 'Semanal', monthly: 'Mensual' } as Record<string, string>)[kind] ?? 'Recurrente'; }
function Meta({ icon, label }: { icon: 'calendar' | 'folder' | 'target' | 'clock' | 'repeat' | 'bell'; label: string }) { return <View style={styles.meta}><FocoIcon name={icon} size={17} color={foco.colors.muted} /><Text style={styles.metaText} numberOfLines={1}>{label}</Text></View>; }
function Action({ icon, label, onPress, primary = false }: { icon: 'play' | 'tomorrow' | 'copy'; label: string; onPress: () => void; primary?: boolean }) { return <Pressable onPress={onPress} style={({ pressed }) => [styles.action, primary && styles.actionPrimary, pressed && pressedStyle]}><FocoIcon name={icon} size={20} color={primary ? foco.colors.bg : foco.colors.text} /><Text style={[styles.actionText, primary && styles.actionTextPrimary]}>{label}</Text></Pressable>; }
function Section({ title, detail, children }: { title: string; detail?: string; children: React.ReactNode }) { return <View style={styles.section}><View style={styles.sectionHeader}><Text style={styles.sectionTitle}>{title}</Text>{detail ? <Text style={styles.sectionDetail}>{detail}</Text> : null}</View>{children}</View>; }
...
const styles = StyleSheet.create({
  ...
  title: { flex: 1, color: foco.colors.text, fontSize: 28, lineHeight: 34, fontWeight: '700', letterSpacing: -0.7 },
  ...
  sessionDate: { color: foco.colors.text, fontSize: 14.5, fontWeight: '500' },
