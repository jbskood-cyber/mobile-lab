...
export function DistributionList({ items, emptyCopy }: { items: Array<{ id: string; label: string; focusSeconds: number }>; emptyCopy: string }) {
  ...
  return <View style={styles.list}>{items.slice(0, 6).map((item, index) => { const percentage = total === 0 ? 0 : Math.round((item.focusSeconds / total) * 100); return <View key={item.id} style={styles.row} accessibilityLabel={`${item.label}: ${formatDuration(item.focusSeconds, true)}, ${percentage}%`}><Text style={styles.rank}>{index + 1}</Text><View style={styles.copy}><View style={styles.titleLine}><Text style={styles.label} numberOfLines={1}>{item.label}</Text><Text style={styles.value}>{formatDuration(item.focusSeconds, true)}</Text></View><View style={styles.track}><View style={[styles.fill, { width: `${Math.max(3, (item.focusSeconds / max) * 100)}%` }]} /></View></View><Text style={styles.percent}>{percentage}%</Text></View>; })}</View>;
}
...
const styles = StyleSheet.create({
  ...
  label: { flex: 1, color: foco.colors.text, fontSize: 13.5, fontWeight: '500' },
