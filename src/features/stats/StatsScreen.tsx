...
export function StatsScreen() {
  ...
  const projectColors = useMemo(() => new Map(state.projects.map((project) => [project.id, resolveProjectColor(project.color, theme.mode)])), [state.projects, theme.mode]);
  const taskProjects = useMemo(() => new Map(state.tasks.map((task) => [task.id, task.projectId])), [state.tasks]);
  ...
  return (
    <FocoScreen title="Progreso" subtitle="Datos que ayudan a ajustar tu siguiente día." screenKey="stats" rightIcon="calendar" rightAccessibilityLabel="Volver al periodo actual" onRightPress={() => setAnchor(Date.now())}>
      ...
      <View style={[styles.plan, { borderColor: theme.colors.borderSoft }]}><View style={styles.planNumbers}><PlanValue value={stats.completedPomodoros} label="Completados" /><PlanValue value={stats.plannedPomodoros} label="Planificados" align="right" /></View><View style={[styles.track, { backgroundColor: theme.colors.panelStrong }]}><View style={[styles.fill, { width: `${Math.round(completionRatio * 100)}%`, backgroundColor: theme.colors.text }]} /></View><Text style={[styles.goalCopy, { color: theme.colors.muted }]}>Objetivo de tiempo alcanzado: {Math.round(stats.goalRate * 100)}%</Text></View>

      <SectionTitle title="Por proyecto" detail={String(projects.length)} />
      <Distribution items={projects.map((item) => ({ id: item.projectId, label: item.projectName, value: item.focusSeconds, color: projectColors.get(item.projectId) ?? theme.colors.text }))} max={maxDistribution} empty="Las sesiones ligadas a proyectos aparecerán aquí." />
      <SectionTitle title="Por tarea" detail={String(tasks.length)} />
      <Distribution items={tasks.map((item) => { const projectId = taskProjects.get(item.taskId); return { id: item.taskId, label: item.taskTitle, value: item.focusSeconds, color: projectId ? projectColors.get(projectId) ?? theme.colors.text : theme.colors.text }; })} max={maxDistribution} empty="Selecciona una tarea antes de iniciar el temporizador." />
...
function Distribution({ items, max, empty }: { items: Array<{ id: string; label: string; value: number; color: string }>; max: number; empty: string }) { const theme = useFocoTheme(); return items.length === 0 ? <Text style={[styles.emptyText, { color: theme.colors.muted }]}>{empty}</Text> : <View style={[styles.distribution, { borderColor: theme.colors.borderSoft }]}>{items.map((item) => <View key={item.id} style={styles.distributionRow}><View style={styles.distributionTop}><View style={styles.distributionIdentity}><View style={[styles.distributionMarker, { backgroundColor: item.color }]} /><Text style={[styles.distributionLabel, { color: theme.colors.text }]} numberOfLines={1}>{item.label}</Text></View><Text style={[styles.distributionTime, { color: theme.colors.muted }]}>{formatDuration(item.value, true)}</Text></View><View style={[styles.distributionTrack, { backgroundColor: theme.colors.panelStrong }]}><View style={[styles.distributionFill, { width: `${Math.max(3, item.value / max * 100)}%`, backgroundColor: item.color }]} /></View></View>)}</View>; }
...
const styles = StyleSheet.create({
  ...
  periodText: { fontFamily: 'InstrumentSans_600SemiBold', fontSize: 11, lineHeight: 14 },
  ...
  anchorLabel: { minWidth: 150, textAlign: 'center', fontFamily: 'InstrumentSans_500Medium', fontSize: 11.5, lineHeight: 15 },
  ...
  metricValue: { fontFamily: 'InstrumentSans_600SemiBold', fontSize: 16, lineHeight: 20, fontVariant: ['tabular-nums'] },
  metricLabel: { fontFamily: 'InstrumentSans_400Regular', fontSize: 9, lineHeight: 12 },
  ...
  emptyHeroTitle: { fontFamily: 'InstrumentSans_700Bold', fontSize: 12.5, lineHeight: 16 },
  emptyHeroText: { fontFamily: 'InstrumentSans_400Regular', fontSize: 9.5, lineHeight: 13, marginTop: 2, opacity: 0.72 },
  ...
  barLabel: { height: 17, fontFamily: 'InstrumentSans_400Regular', fontSize: 8.5, lineHeight: 12, paddingTop: 3 },
  trendFooter: { minHeight: 36, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  smallLabel: { fontFamily: 'InstrumentSans_400Regular', fontSize: 10.5, lineHeight: 14 },
  smallValue: { fontFamily: 'InstrumentSans_600SemiBold', fontSize: 11, lineHeight: 14, fontVariant: ['tabular-nums'] },
  ...
  planValue: { fontFamily: 'InstrumentSans_600SemiBold', fontSize: 17, lineHeight: 21, fontVariant: ['tabular-nums'] },
  planLabel: { fontFamily: 'InstrumentSans_400Regular', fontSize: 9.5, lineHeight: 12, marginTop: 1 },
  track: { height: 4, borderRadius: 2, marginTop: 9, overflow: 'hidden' },
  fill: { height: 4, borderRadius: 2 },
  goalCopy: { fontFamily: 'InstrumentSans_400Regular', fontSize: 9.5, lineHeight: 13, marginTop: 6 },
  ...
  distributionIdentity: { flex: 1, minWidth: 0, flexDirection: 'row', alignItems: 'center', gap: 6 },
  distributionMarker: { width: 6, height: 6, borderRadius: 3 },
  distributionLabel: { flex: 1, fontFamily: 'InstrumentSans_500Medium', fontSize: 11.5, lineHeight: 15 },
  distributionTime: { fontFamily: 'InstrumentSans_500Medium', fontSize: 10.5, lineHeight: 14, fontVariant: ['tabular-nums'] },
  ...
  productiveRank: { fontFamily: 'InstrumentSans_700Bold', fontSize: 9, lineHeight: 12 },
  productiveHour: { fontFamily: 'InstrumentSans_600SemiBold', fontSize: 13, lineHeight: 17, fontVariant: ['tabular-nums'] },
  productiveTime: { fontFamily: 'InstrumentSans_400Regular', fontSize: 9, lineHeight: 12 },
  ...
  dayLabel: { width: 13, height: 10, fontFamily: 'InstrumentSans_400Regular', fontSize: 7.5, lineHeight: 10 },
  ...
  session: { minHeight: 51, flexDirection: 'row', alignItems: 'center', gap: 8, borderBottomWidth: StyleSheet.hairlineWidth },
  sessionMarker: { width: 7, height: 7, borderRadius: 4 },
  sessionCopy: { flex: 1, minWidth: 0 },
  sessionTitle: { fontFamily: 'InstrumentSans_500Medium', fontSize: 11.5, lineHeight: 15 },
  sessionMeta: { fontFamily: 'InstrumentSans_400Regular', fontSize: 9.5, lineHeight: 13, marginTop: 2 },
  sessionTime: { fontFamily: 'InstrumentSans_600SemiBold', fontSize: 11, lineHeight: 14, fontVariant: ['tabular-nums'] },
  emptyText: { fontFamily: 'InstrumentSans_400Regular', fontSize: 10.5, lineHeight: 15, paddingVertical: 16 },
});
