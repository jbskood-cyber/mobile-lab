    ...
    <View>
      <View style={styles.periods}>
        {(['day', 'week', 'month'] as AnalyticsPeriod[]).map((item) => (
          <Pressable key={item} accessibilityRole="radio" accessibilityState={{ checked: period === item }} onPress={() => { onPeriod(item); hapticSelection(); }} style={({ pressed }) => [styles.period, period === item && styles.periodActive, pressed && pressedStyle]}>
            <Text style={[styles.periodText, period === item && styles.periodTextActive]}>{labels[item]}</Text>
          </Pressable>
        ))}
      </View>
      <View style={styles.anchorRow}>
        <Pressable accessibilityRole="button" accessibilityLabel="Periodo anterior" onPress={() => move(-1)} style={({ pressed }) => [styles.arrow, pressed && pressedStyle]}><FocoIcon name="chevron-left" size={19} color={foco.colors.text} /></Pressable>
        <Text style={styles.anchor}>{periodLabel(period, anchor)}</Text>
        <Pressable accessibilityRole="button" accessibilityLabel="Periodo siguiente" onPress={() => move(1)} style={({ pressed }) => [styles.arrow, pressed && pressedStyle]}><FocoIcon name="chevron-right" size={19} color={foco.colors.text} /></Pressable>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  periodText: { color: foco.colors.muted, fontSize: 13.5, fontWeight: '550' },
