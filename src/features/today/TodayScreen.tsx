...
export function TodayScreen() {
  ...
  const renderRows = (tasks: Task[]) => tasks.map((task) => (
    <TaskRow key={task.id} task={task} projectName={projectMap.get(task.projectId) ?? 'Sin proyecto'} completedPomodoros={taskPomodoros.get(task.id) ?? 0} onPress={() => openTask(task)} onToggle={() => toggle(task)} />
  ));

  return (
    <>
      <FocoScreen title="Hoy" subtitle={todayLabel()} screenKey="index" rightIcon="plus" rightAccessibilityLabel="Crear tarea" onRightPress={() => { setEditingTask(undefined); setEditorOpen(true); }}>
        {storageError ? <View style={styles.warning}><Text style={styles.warningText}>{storageError}</Text></View> : null}

        <View style={styles.summary}>
          <Summary value={String(buckets.today.length + buckets.overdue.length)} label="Por hacer" />
          <Summary value={String(completedToday.length)} label="Completadas" />
          <Summary value={formatDuration(focusToday, true)} label="Enfoque" />
        </View>

        <View style={styles.quickAdd}>
          <Pressable accessibilityRole="button" accessibilityLabel="Escribir tarea" onPress={() => inputRef.current?.focus()} style={({ pressed }) => [styles.quickIcon, pressed && pressedStyle]}><FocoIcon name="plus" size={22} color={foco.colors.muted} /></Pressable>
          <TextInput ref={inputRef} value={draft} onChangeText={setDraft} onSubmitEditing={submitQuick} placeholder="Añadir para hoy" placeholderTextColor={foco.colors.subtle} returnKeyType="done" autoCapitalize="sentences" style={styles.quickInput} />
          <Pressable accessibilityRole="button" accessibilityLabel="Guardar tarea" disabled={!draft.trim()} onPress={submitQuick} style={({ pressed }) => [styles.quickSave, !draft.trim() && styles.disabled, pressed && pressedStyle]}><FocoIcon name="check" size={18} color={foco.colors.bg} /></Pressable>
        </View>

        <SectionTitle title="Ahora" detail={nextAction ? projectMap.get(nextAction.projectId) : undefined} />
        {nextAction ? (
          <Pressable accessibilityRole="button" accessibilityLabel={`Enfocarse en ${nextAction.title}`} onPress={() => startFocus(nextAction)} style={({ pressed }) => [styles.nextAction, pressed && pressedStyle]}>
            <View style={styles.nextCopy}><Text style={styles.nextTitle} numberOfLines={2}>{nextAction.title}</Text><Text style={styles.nextMeta}>{nextAction.estimatedPomodoros} {nextAction.estimatedPomodoros === 1 ? 'bloque estimado' : 'bloques estimados'}</Text></View>
            <View style={styles.focusButton}><FocoIcon name="play" size={22} color={foco.colors.bg} /></View>
          </Pressable>
        ) : <Empty title="Tu día está despejado" copy="Añade una tarea o elige algo desde Agenda." />}
      ...
      <TaskEditorSheet visible={editorOpen} task={editingTask} defaultDueAt={Date.now()} onClose={() => setEditorOpen(false)} />
    </>
  );
}
...
const styles = StyleSheet.create({
  ...
  summaryItem: { flex: 1 },
  summaryValue: { color: foco.colors.text, fontSize: 20, fontWeight: '650', fontVariant: ['tabular-nums'] },
  ...
  nextTitle: { color: foco.colors.text, fontSize: 18, lineHeight: 23, fontWeight: '650' },
